...
function update_mission_panel(player)
  ...
  if (storage.nullius_mission_show[player.index]) then
...
function update_mission_player(player)
  if (storage.nullius_mission_status ~= nil) then
    if (storage.nullius_mission_complete) then
      storage.nullius_mission_show[player.index] = false
    ...
    elseif (storage.nullius_mission_show[player.index] == nil) then
      storage.nullius_mission_show[player.index] = true
      if mod_gui.get_button_flow(player).nullius_mission_button then
        mod_gui.get_button_flow(player).nullius_mission_button.destroy()
      end
...
function init_mission_global()
  ...
  for i = 1, 14 do
    if (storage.nullius_mission_count[i] == nil) then
      storage.nullius_mission_count[i] = 0
      storage.nullius_mission_status[i] = 0
    end
  end
...
function set_mission_goal(goal, amount, force)
  ...
  local count = storage.nullius_mission_count
  local status = storage.nullius_mission_status
  ...
  if ((goal >= 3) and (goal <= 5)) then
	...
	storage.nullius_oxygen_bio_target = (108 * total_oxygen) / (150 - total_oxygen)
  end
  ...
  local overall = storage.nullius_mission_status[2]
  for i = 1, 13 do
    overall = overall + math.min(100, storage.nullius_mission_status[i])
  end
  storage.nullius_mission_status[14] = (math.floor(100 * (overall / 14)) / 100)
...
function bump_mission_goal(goal, amount, force)
  if (amount ~= 0) then
    ...
    set_mission_goal(goal, (storage.nullius_mission_count[goal] + amount), force)
  end
end
...
function update_oxygen()
  if (storage.nullius_mission_status == nil) then return end
  if (storage.nullius_mission_complete) then return end
  local prev_status = storage.nullius_mission_status[2]
  if (prev_status >= 100) then return end

  local current = (((storage.nullius_oxygen_bio_current * 8191) +
      storage.nullius_oxygen_bio_target) / 8192)
  storage.nullius_oxygen_bio_current = current
...
function migrate_oxygen()
  ...
  storage.nullius_oxygen_legacy = math.max(0, (((2 * consumed) - produced)/3))
  if (storage.nullius_mission_status == nil) then return end
  storage.nullius_oxygen_bio_target = (storage.nullius_mission_count[2] / 1.5)
  storage.nullius_oxygen_bio_current = (storage.nullius_oxygen_bio_target / 2)
